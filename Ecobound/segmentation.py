...
# 允许 Spatial Analyst 扩展模块和 3D Analyst 扩展模块
...
def threshold_segmentation(input_raster, threshold):
    ...
    valid_raster = np.where(mask_valid, raster, np.nan)
    ...
    # 释放中间大数组
    for obj_name in [
        "low_mask", "high_mask", "low_labeled", "high_labeled",
        "valid_raster", "mask_valid", "raster"
    ]:
...
def save_raster(array, template_raster_path, output_raster_path):
    """保存 numpy 数组为栅格；uint8 用 0 作 NoData，浮点用 NaN。"""
...
def _delete_if_exists(path_or_name):
    try:
        if arcpy.Exists(path_or_name):
            arcpy.Delete_management(path_or_name)
    except Exception:
        pass


def _derive_companion_path(path_str, suffix):
...
def extract_and_smooth_boundary(original_raster, segmented_raster, threshold, output_boundary_shapefile):
    """
    提取并平滑栅格数据中值为 1 和 2 的边界线，并保存为 Shapefile。

    v3.30 patch:
    - Basic 许可证下 PolygonToLine 不可用时，不再让整个流程失败；
    - 始终保留 polygon 结果；
    - 若 line 生成失败，则将 polygon 输出到用户指定路径，并给出 warning。
    """
    region1_polygon = "in_memory/region1_polygon"
    region2_polygon = "in_memory/region2_polygon"
    merged_regions = "in_memory/merged_regions"
    ...
    # 伴随输出：永远保留 retained regions polygon
    output_regions_shapefile = _derive_companion_path(output_boundary_shapefile, "_regions")
    ...
    try:
        ...
        # 始终保留 polygon 结果（供 Basic fallback 与 zoning 使用）
        _delete_if_exists(output_regions_shapefile)
        arcpy.management.Merge([region1_polygon, region2_polygon], merged_regions)
        arcpy.management.CopyFeatures(merged_regions, output_regions_shapefile)

        try:
            # 提取两个多边形的边界（Advanced/Standard 可用；Basic 可能失败）
            ...
            arcpy.Intersect_analysis(
                [region1_boundary, region2_boundary],
                initial_boundary_intersect,
                "ALL",
                "",
                "LINE",
            )
            ...
            arcpy.management.SelectLayerByLocation(
                "contour_layer", "INTERSECT", "initial_boundary_layer"
            )

            # 保存最终线结果
            _delete_if_exists(output_boundary_shapefile)
            arcpy.management.CopyFeatures("contour_layer", output_boundary_shapefile)
            print(f"✅ Boundary line saved: {output_boundary_shapefile}")

        except Exception as e_line:
            # PolygonToLine 或后续线操作失败：直接退回 polygon 输出
            _delete_if_exists(output_boundary_shapefile)
            arcpy.management.CopyFeatures(merged_regions, output_boundary_shapefile)
            msg = (
                "PolygonToLine/line-extraction is unavailable under the current ArcGIS "
                f"license or environment. Retained boundary polygons were saved instead: {output_boundary_shapefile}. "
                f"Detailed error: {e_line}"
            )
            try:
        ...
        for tmp in [
            ...
            initial_boundary_intersect,
            contour_lines,
            "initial_boundary_layer",
            "contour_layer",
        ]:
            _delete_if_exists(tmp)
...
def generate_natural_boundary(input_raster, break_value, output_boundary_shapefile):
    ...
    # 提取并平滑自然地理界线（若 line 失败，则自动输出 polygon）
    extract_and_smooth_boundary(
        input_raster,
        segmented_raster_path,
        break_value,
        output_boundary_shapefile,
    )
